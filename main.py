...
def run_lr_W4_Nu2_Minf_Na1_2500(): 
    for i in range(40):
        ...
        for u in range(2,101):
            reslist[u-2]+= random_m4_variance(noisy_W(4, 0), na=2, NuA=2, NuB=2, Nm=-1, N_iid=50,  device=device, scheme="l", qU=qU,N_average=u**2//4)/40
    th.save(th.tensor(reslist), "./output/lr_W4_Nu2_Minf_Na1_2500")
def run_lr_W4_Nu2_100_Minf_Na1(): 
    for i in range(20):
        print(i)
        for u in range(2,101):
            reslist[u-2]+= random_m4_variance(noisy_W(4, 0), na=2, NuA=u, NuB=u, Nm=-1, N_iid=100,  device=device, scheme="l", qU=qU)/20
    th.save(th.tensor(reslist), "./output/lr_W4_Nu2_100_Minf_Na1")
def run_lr_W4_Nu2_100_M20_Na1(): 
    for i in range(20):
        print(i)
        for u in range(2,101):
            reslist[u-2]+= random_m4_variance(noisy_W(4, 0), na=2, NuA=u, NuB=u, Nm=20, N_iid=100,  device=device, scheme="l", qU=qU)/20
...
def run_gr_W4_Nu2_Minf_Na1_2500(): 
    for i in range(40):
        print(i)
        for u in range(2,101):
            reslist[u-2]+= random_m4_variance(noisy_W(4, 0), na=2, NuA=2, NuB=2, Nm=-1, N_iid=50,  device=device, scheme="g", qU=qU,N_average=u**2//4)/40
    th.save(th.tensor(reslist), "./output/gr_W4_Nu2_Minf_Na1_2500")
def run_gr_W4_Nu2_100_Minf_Na1(): 
    for i in range(20):
        print(i)
        for u in range(2,101):
            reslist[u-2]+= random_m4_variance(noisy_W(4, 0), na=2, NuA=u, NuB=u, Nm=-1, N_iid=100,  device=device, scheme="g", qU=qU)/20
    th.save(th.tensor(reslist), "./output/gr_W4_Nu2_100_Minf_Na1")
def run_gr_W4_Nu2_100_M20_Na1(): 
    for i in range(20):
        print(i)
        for u in range(2,101):
            reslist[u-2]+= random_m4_variance(noisy_W(4, 0), na=2, NuA=u, NuB=u, Nm=20, N_iid=100,  device=device, scheme="g", qU=qU)/20
    th.save(th.tensor(reslist), "./output/gr_W4_Nu2_100_M20_Na1")

#! plot4
def run_gr_W4_Nu10_M1_100_Na1(): 
    for m in range(1,100):
        reslist[m-1]+= random_m4_variance(noisy_W(4, 0), na=2, NuA=10, NuB=10, Nm=m, N_iid=3000,  device=device, scheme="g", qU=qU)
    th.save(th.tensor(reslist), "./output/gr_W4_Nu10_M1_100_Na1")

def run_lr_W4_Nu10_M1_100_Na1(): 
    for m in range(1,100):
        reslist[m-1]+= random_m4_variance(noisy_W(4, 0), na=2, NuA=10, NuB=10, Nm=m, N_iid=3000,  device=device, scheme="l", qU=qU)
    th.save(th.tensor(reslist), "./output/lr_W4_Nu10_M1_100_Na1")

def run_gr_W4_Nu20_M1_100_Na1(): 
    for m in range(1,100):
        reslist[m-1]+= random_m4_variance(noisy_W(4, 0), na=2, NuA=20, NuB=20, Nm=m, N_iid=2000,  device=device, scheme="g", qU=qU)
...
def run_lr_W2_9_Nu3_M10_Na1(): 
    for time in range(1000):
        for n in range(2,10):
            reslist[n-2]+= random_m4_variance(noisy_W(n, 0), na=n//2, NuA=3, NuB=3, Nm=10, N_iid=1,  device=device, scheme="l", qU=qU)/1000
